 ...
 *
 * 所谓编译执行，就是通过即时编译器（Just in Time,JIT)将字节码转换为本地机器码来执行，现代JVM会根据代码热点来生成 相应的本地机器码
 *
 *
 * 基于栈的指令集与基于寄存器的指令集之间的关系： 1. JVM执行指令时基于栈的指令集：内存中操作，有入栈和操作两种 优点：可移植性好，字节码可以移植
 * 缺点：速度会慢些，指令数量多，入栈出栈操作可能寄存器一个指令就搞定了
 *
 * 2. 基于寄存器的指令集：和硬件相关,不可移植， cpu，cpu高速缓冲区执行
 *
 * eg: 2-1 1. iconst_1 将1压到栈顶 2. iconst_2 将2压到栈顶 3. isub 将栈顶和栈顶下面的一个元素弹出来，将栈顶-（栈顶-1）的结果在压入栈顶 4.
 * istroe_0 将栈顶元素放到局部变量表的store中
 */
