 ...
 *
 * 1. invokeinterface: 调用接口中的方法，实际上是在运行期决定的，决定到底调用实现该接口的哪个对象的特定方法。运行期查找具体的类 2. invokestatic: 调用静态方法
 * 3. invokespecial: 调用自己的私有方法、构造方法（<init>) 以及父类的方法 4. invokevirtual: 调用虚方法，运行期动态查找的过程，多态 5.
